...
namespace Advent
{
    class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            Console.WriteLine("Total count: " + input.Sum(x => x.Length));
        }
        ...
        private static void ReadInput()
        {
            ...
            foreach (var line in lines)
            {
                ...
                {
                    input.Add(GetReoccurringCharacters(groupAnswers));
        ...
        private static string GetReoccurringCharacters(List<string> groupAnswers)
        {
            var finalGroupAnswer = "";
            foreach (var character in groupAnswers[0])
            {
                var includeCharacter = true;
                for (int i = 1; i < groupAnswers.Count; i++)
                {
                    if (!groupAnswers[i].Contains(character))
                        includeCharacter = false;
                }
                if (includeCharacter)
                    finalGroupAnswer += character;
            }

            return finalGroupAnswer;
        }
    }
}
